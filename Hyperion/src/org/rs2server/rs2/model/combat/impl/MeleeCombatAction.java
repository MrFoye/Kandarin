...
public class MeleeCombatAction extends AbstractCombatAction {
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		int damage = damage(maxHit, attacker, victim, attacker.getCombatState().getAttackType(), Skills.ATTACK, Prayers.PROTECT_FROM_MELEE, special, false);
		...
		if(!special) {
			...
			if(attacker.getEquipment().get(Equipment.SLOT_WEAPON) != null && attacker.getEquipment().get(Equipment.SLOT_WEAPON).getEquipmentDefinition() != null) {
				int length = attacker.getEquipment().get(Equipment.SLOT_WEAPON).getEquipmentDefinition().getAnimation().getAttacks().length;
				if(attackAnimationIndex >= length) {
					attackAnimationIndex = length-1;
				}				
			}
			attacker.playAnimation(
					(attacker.getEquipment().get(Equipment.SLOT_WEAPON) != null && attacker.getEquipment().get(Equipment.SLOT_WEAPON).getEquipmentDefinition() != null)
					? attacker.getEquipment().get(Equipment.SLOT_WEAPON).getEquipmentDefinition().getAnimation().getAttack(attackAnimationIndex)
		...
		if(victim.getCombatState().getPoisonDamage() < 1 && random.nextInt(11) == 3 && victim.getCombatState().canBePoisoned()) {
			if(attacker.getEquipment().get(Equipment.SLOT_WEAPON) != null && attacker.getEquipment().get(Equipment.SLOT_WEAPON).getEquipmentDefinition() != null) {
				if(attacker.getEquipment().get(Equipment.SLOT_WEAPON).getEquipmentDefinition().getPoisonType() != PoisonType.NONE) {
					victim.getCombatState().setPoisonDamage(attacker.getEquipment().get(Equipment.SLOT_WEAPON).getEquipmentDefinition().getPoisonType().getMeleeDamage(), attacker);
				...
				World.getWorld().submit(new Tickable(1) {
					...
					public void execute() {
						for(int i = 1; i < weapon.getEquipmentDefinition().getSpecialHits(); i++) {
							int dmg = damage(maxHit, attacker, victim, attacker.getCombatState().getAttackType(), Skills.ATTACK, Prayers.PROTECT_FROM_MELEE, true, false);
	...
	public int distance(Mob attacker) {
		return 1; //TODO: Halberds/spears = 2 squares
	}
	...
	public void addExperience(Mob attacker, int damage) {
		...
		for(int i = 0; i < attacker.getCombatState().getCombatStyle().getSkills().length; i++) {
			attacker.getSkills().addExperience(attacker.getCombatState().getCombatStyle().getSkill(i),
					(attacker.getCombatState().getCombatStyle().getExperience(i) * damage) * Constants.EXP_MODIFIER);
		}
	}
